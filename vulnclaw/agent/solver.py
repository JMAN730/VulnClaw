...
def _system_prompt(agent: AgentContext, state: AgentState) -> str:
    ...
    return (
        ...
        "When the latest user turn is a question, readiness check, or status check "
        "(e.g. 'ready to begin?', 'are we ready?', 'what's the plan?'), answer in plain text "
        "first: current scope summary, intended next asset, blockers, then a short plan. "
        "Do not call agent_run or recon/attack tools in that same turn unless the user "
        "already gave a clear operational go-ahead. After you answer, wait for confirmation "
        "or treat an explicit yes/start as the green light.\n"
        f"{fanout_guidance}"
