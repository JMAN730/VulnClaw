"""
VulnBot MITRE ATT&CK Mapping Engine
=====================================
...
# Maps VulnBot's allowed tools to the ATT&CK techniques they implement.
...
def _build_navigator_layer(
    ...
    layer = {
        ...
        "domain": _NAV_DOMAIN,
        "description": description or f"Generated by VulnBot v{__version__}",
        ...
        "metadata": [
            {"name": "generator", "value": f"VulnBot v{__version__}"},
...
class AttackMapper:
    """Maps VulnBot findings and tool usage to MITRE ATT&CK techniques.
    ...
    def generate_navigator_layer(
        ...
        Args:
            ...
            name: Layer name (default: "VulnBot: <target>")
        ...
        """
        if not name:
            name = f"VulnBot: {report.target}" if report.target else "VulnBot Assessment"
        if not description:
            description = (
    ...
    def format_report(report: AttackReport) -> str:
        ...
        lines.extend([
            ...
            "**Source:** 🔍 Finding | 🔧 Tool",
            f"\n*Generated by VulnBot v{__version__}*",
        ])
