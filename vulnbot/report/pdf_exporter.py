"""PDF export for VulnBot reports.

Spec §5.3: consume VulnBot's existing report (the markdown produced by
``vulnbot.report.generator.generate_report``) and render it to a styled PDF —
not a parallel report builder. ``reportlab`` is an optional extra
(``vulnbot[pdf]``); importing this module is fine without it, but calling
...
_PDF_EXTRA_HINT = (
    "PDF export requires reportlab. Install the extra with: pip install 'vulnbot[pdf]'"
)
...
def export_pdf(
    ...
    """Render a VulnBot markdown report to a PDF file.
